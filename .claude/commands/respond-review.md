...
```
⚠️ レビュー結果が見つかりません。
  PR型: `8moku review pr {NUMBER}` を実行してください
  diff型: `8moku review diff` を実行してください
```
...
| JSONLファイルが存在しない | `⚠️ レビュー結果が見つかりません。8moku review pr <番号> または 8moku review diff を実行してください` |
